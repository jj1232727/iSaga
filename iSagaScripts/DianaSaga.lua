...
LocalCallbackAdd("Draw", function()
    ...
    if Saga.Drawings.Q.Enabled:Value() then 
        Draw.Circle(myHero.pos, 830, 0, Saga.Drawings.Q.Color:Value())
    end

    if Saga.Drawings.W.Enabled:Value() then 
        Draw.Circle(myHero.pos, 200, 0, Saga.Drawings.W.Color:Value())
    end


    if Saga.Drawings.R.Enabled:Value() then 
        Draw.Circle(myHero.pos, 825, 0, Saga.Drawings.R.Color:Value())
    end
    
    for i = 1, TotalHeroes do 
        local hero = _EnemyHeroes[i]
        if validTarget(hero) then
            if GetFullCombo(hero) + GetDamage(hero,"sheen") + GetDamage(hero,"lich") > hero.health + hero.shieldAD + hero.shieldAP then
...
LocalCallbackAdd("Tick", function()
    ...
    GetEnemiesinRangeCountR = function(target,range)
        --local closest = 9999
        local cMinion
            for i = 1, SagaMCount() do 
                local minion = SagasBitch(i)
                if minion and not minion.dead and minion.visible and minion.isTargetable then
                    if GetDistanceSqr(myHero.pos, target.pos) < 825 * 825 then 
                        --closest = GetDistanceSqr(myHero.pos, target.pos)
                        cMinion = minion
                    end
                    if GetDistanceSqr(minion.pos, target.pos) < 825 * 825 and GotBuff(minion, "dianamoonlight") == 1 then
    ...
    Combo = function()
        ...
        if target then
            ...
            if Game.CanUseSpell(3) == 0 then
                ...
                local targetExtend = GetEnemiesinRangeCountR(target, 825)
                local targetExtend2 = GetEnemiesinRangeCountR(target, 400)
                if targetExtend and Saga.Combo.gap:Value() then
                    if target.pos:DistanceTo() >= 825 then
                        Control.CastSpell(HK_R, targetExtend)
                    end
                end

                if targetExtend2 and Saga.Combo.gap:Value() then
                    if target.pos:DistanceTo() >= 825 then
                        Control.CastSpell(HK_R, targetExtend2)
                    end
                end
            ...
            if Game.CanUseSpell(1) == 0 and GetDistanceSqr(myHero,target) < (200+target.boundingRadius) * (200+target.boundingRadius) then
                if Saga.Combo.UseW:Value() then
                    Control.CastSpell(HK_W)
                end
            end
            
            if Game.CanUseSpell(2) == 0 and GetDistanceSqr(myHero,target) < 450 * 450 then
            ...
            if minion then
                if minion.team == 300 - myHero.team and minion.isTargetable and minion.visible and not minion.dead then
                    if Saga.Clear.UseQ:Value() then
                        if Game.CanUseSpell(0) == 0 and minion.pos:DistanceTo() < 830 then
                            CastSpell(HK_Q, minion,500)
                        end
                    end
                    if Saga.Clear.UseW:Value() then
                        if Game.CanUseSpell(1) == 0 and GetDistanceSqr(myHero,minion) < (200+minion.boundingRadius) * (200+minion.boundingRadius) then
                            Control.CastSpell(HK_W)
                        end
                    end
                end
    ...
    GetFullCombo = function(target)
        local spells = {HK_Q,HK_W,HK_R, "passive"}
        local damage = 0
        for i = 1, #spells do 
            local s = spells[i]
    ...
    function CalcPhysicalDamage(source, target, amount)
      ...
      PassivePercentMod = function(source, target, amount, damageType)
        ...
        local basedamage = 0
        local items = checkItems()
        local sheen = items[3057]
        local lich = items[3100]
        local passiveDMG = (({[1]=20,[2]=25,[3]=30,[4]=35,[5]=40,[6]=50,[7]=60,[8]=70,[9]=80,[10]=90,[11]=105,[12]=120,[13]=135,[14]=155,[15]=175,[16]=200,[17]=225,[18]=250})[myHero.levelData.lvl])

        if Game.CanUseSpell(0) == 0 and spell == HK_Q then
            basedamage = CalcMagicalDamage(myHero,target, (myHero:GetSpellData(_Q).level* 35 + 35) + (.7 * myHero.ap))
        end
        
      
        if Game.CanUseSpell(1) == 0 and spell == HK_W then
            basedamage = CalcMagicalDamage(myHero,target, (myHero:GetSpellData(_W).level* 12 + 10) + (.2 * myHero.ap))
        end
      
        if Game.CanUseSpell(3) == 0 and spell == HK_R then
            basedamage = CalcMagicalDamage(myHero,target, (myHero:GetSpellData(_R).level* 60 + 40) + (.6 * myHero.ap)) * 2
        end

        if sheen and spell == "sheen" then
            basedamage = CalcMagicalDamage(myHero, target, myHero.totalDamage)
        end
        ...
        if spell == "passive" then
            basedamage = CalcMagicalDamage(myHero, target, passiveDMG + 0.8 * myHero.ap)
        end
