...
LocalCallbackAdd("Draw", function()
    if Saga.Drawings.Q.Enabled:Value() then 
        Draw.Circle(myHero.pos, 925, 0, Saga.Drawings.Q.Color:Value())
    end

    if Saga.Drawings.W.Enabled:Value() then 
        Draw.Circle(myHero.pos, 950, 0, Saga.Drawings.W.Color:Value())
    end

    if Saga.Drawings.E.Enabled:Value() then 
        Draw.Circle(myHero.pos, 425, 0, Saga.Drawings.E.Color:Value())
    end

    if Saga.Drawings.R.Enabled:Value() then 
        Draw.Circle(myHero.pos, 1000, 0, Saga.Drawings.R.Color:Value())
    end
...
Combo = function()
    ...
    if target and validTarget(target) then
        SIGroup(target)
        if Saga.Combo.UseR:Value() and GetDamage(target, HK_R) > target.health and Game.CanUseSpell(3) == 0 and GetDistance(myHero,target) < 1000 then
            local aim = GetPred(target,2100,.25+Game.Latency()/1000)
        ...
        if target.pos:DistanceTo() < 950 and Saga.Combo.UseW:Value() and Game.CanUseSpell(1) == 0 then
            local aim = GetPred(target,1000,.25+Game.Latency()/1000)
            Control.CastSpell(HK_W, aim)
            _G.SDK.Orbwalker:__OnAutoAttackReset()
        end

        if target.pos:DistanceTo() > myHero.range and target.pos:DistanceTo() < myHero.range + 425 and Saga.Combo.UseE:Value() and Game.CanUseSpell(2) == 0 
        or myHero.hudAmmo < 2 and Saga.Combo.UseE:Value() and Game.CanUseSpell(2) == 0 and target.pos:DistanceTo() < myHero.range + 425 then
            if Saga.Combo.Et:Value() then
        ...
        if target.pos:DistanceTo() < 925 and Saga.Combo.UseQ:Value() and Game.CanUseSpell(0) == 0 then
            local aim = GetPred(target,1000,.65+Game.Latency()/1000)
            --if aim then
                --local segment = LineSegment(Point(myHero.pos), Point(aim))
                --if MapPosition:intersectsWall(segment) then return end
...
Harass = function()
    target = GetTarget(2000)
    if target then 
        if target.pos:DistanceTo() < 950 and Saga.Harass.UseW:Value() and Game.CanUseSpell(1) == 0 then
            local aim = GetPred(target,1000,.25+Game.Latency()/1000)
            Control.CastSpell(HK_W, aim)
            _G.SDK.Orbwalker:__OnAutoAttackReset()
        end
        ...
        if target.pos:DistanceTo() > myHero.range and target.pos:DistanceTo() < myHero.range + 425 and Saga.Combo.UseE:Value() and Game.CanUseSpell(2) == 0 
        or myHero.hudAmmo < 2 and Saga.Harass.UseE:Value() and Game.CanUseSpell(2) == 0 and target.pos:DistanceTo() < myHero.range + 425 then
            if Saga.Combo.Et:Value() then
                Control.CastSpell(HK_E, target)
                _G.SDK.Orbwalker:__OnAutoAttackReset()
            else 
        ...
        if target.pos:DistanceTo() < 925 and Saga.Harass.UseQ:Value() and Game.CanUseSpell(0) == 0 then
            local aim = GetPred(target,1000,.65+Game.Latency()/1000)
            --local segment = LineSegment(Point(myHero.pos), Point(aim))
            --if MapPosition:intersectsWall(segment) then return end
            Control.CastSpell(HK_Q, aim)
    ...
    for i = 1, Game.MinionCount() do
        ...
        if minion then
			if minion.pos:DistanceTo() < 1000 and minion.isTargetable and minion.visible and not minion.dead and minion.isEnemy then
                ...
                if minion.pos:DistanceTo() < 925 and Saga.Clear.UseQ:Value() and Game.CanUseSpell(0) == 0 then
                    --local segment = LineSegment(Point(myHero.pos), Point(minion.pos))
                    --if not MapPosition:intersectsWall(segment) then
                        if string.find(minion.name, "SRU_Dragon") or string.find(minion.name, "SRU_Baron") then
                            if minion.health < (GetDamage(minion, HK_Q) * 2 + GetDamage(minion, "smite")) and
                            minion.health > (GetDamage(minion, HK_Q) + GetDamage(minion, "smite"))then
                                return
                            end
                        end
                ...
                if minion.pos:DistanceTo() > myHero.range and minion.pos:DistanceTo() < myHero.range + 425 and Saga.Clear.UseE:Value() and Game.CanUseSpell(2) == 0 
                or myHero.hudAmmo < 2 and Saga.Clear.UseE:Value() and Game.CanUseSpell(2) == 0 and minion.pos:DistanceTo() < myHero.range + 425 then
                    if Saga.Clear.Et:Value() then
                ...
                if minion.pos:DistanceTo() < 950 and Saga.Clear.UseW:Value() and Game.CanUseSpell(1) == 0 then
  ...
  PassivePercentMod = function(source, target, amount, damageType)
    ...
    GetDamage= function(target, spell)
        ...
        if Game.CanUseSpell(3) == 0 and spell == HK_R then
            basedamage = CalcPhysicalDamage(myHero,target, (myHero:GetSpellData(_R).level* 150 + 100) + (1.5 * myHero.bonusDamage))
        end
    
        if Game.CanUseSpell(1) == 0 and spell == HK_W then
            basedamage = CalcPhysicalDamage(myHero,target, (myHero:GetSpellData(_W).level* 50 + 10) + ( .60 * myHero.AP))
        end
    
        if Game.CanUseSpell(0) == 0 and spell == HK_Q then
            basedamage = CalcPhysicalDamage(myHero,target, (myHero:GetSpellData(_Q).level * 15 - 30) + (myHero.totalDamage) + (myHero:GetSpellData(_Q).level * 30 - 55) + ((myHero:GetSpellData(_Q).level * .30 + 10) * myHero.totalDamage ))
        end
        if smiteslot and smitecast then
...
Saga_Menu = 
function()
	Saga = MenuElement({type = MENU, id = "Lee Sin", name = "Saga's Graves: Close But No Cigar ", icon = AIOIcon})
    ...
    Saga.Combo:MenuElement({id = "Et", name = "E To Target", value = true})
    ...
    Saga.Clear:MenuElement({id = "Et", name = "E To Minion/Camps", value = true})
